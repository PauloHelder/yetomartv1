...
const MembersArea: React.FC = () => {
  ...
  const [currentLesson, setCurrentLesson] = useState<any>(allLessons[0] || MOCK_LESSONS[0]);
      ...
      <header className="bg-[#0f172a]/95 backdrop-blur-md border-b border-white/10 px-4 sm:px-6 py-4 flex items-center justify-between z-50 sticky top-0">
        <div className="flex items-center space-x-4 sm:space-x-6">
          ...
          <div className="flex flex-col min-w-0">
            ...
            <h1 className="text-xs sm:text-sm font-black text-white line-clamp-1 uppercase tracking-tighter italic font-serif">{currentLesson.title}</h1>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-2 sm:space-x-6">
           ...
           <button className="bg-yetomart-teal text-white px-4 sm:px-6 py-2 rounded-sm text-[10px] sm:text-xs font-black uppercase tracking-widest hover:bg-yetomart-teal/80 transition-all shadow-lg active:scale-95">
             Concluir Aula
           </button>
           <button 
                ...
                <div className="group relative">
                  ...
                  <div className="absolute top-1/2 -translate-y-1/2 left-4 right-4 flex justify-between pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    <button 
                      ...
                      className={`p-4 rounded-full bg-black/60 text-white backdrop-blur-md pointer-events-auto transition-all hover:bg-yetomart-teal disabled:opacity-0 disabled:pointer-events-none shadow-2xl border border-white/10`}
                    >
                  ...
                  <div className="pb-24">
                      ...
                      <div className="animate-fadeIn">
                        ...
                        {currentLesson.description ? (
                          ...
                          <p className="text-slate-400 leading-relaxed text-lg mb-10 font-medium">
                            Nesta aula do curso <strong className="text-white">{product.title}</strong>, você vai mergulhar fundo nos conceitos práticos de {currentLesson.title.toLowerCase()}. 
                        ...
                        <div className="bg-white/5 border border-white/10 p-8 rounded-sm flex items-center space-x-6 backdrop-blur-sm">
                          <div className="w-14 h-14 bg-yetomart-teal/10 rounded-full flex items-center justify-center text-yetomart-teal shadow-inner">
                          ...
                          currentLesson.attachments.map((att: any) => (
                            <div key={att.id} className="p-5 bg-white/5 border border-white/10 rounded-sm flex items-center justify-between hover:bg-white/10 transition-colors cursor-pointer group">
                               <div className="flex items-center space-x-4">
                                 <div className="p-3 bg-yetomart-teal/20 rounded-sm text-yetomart-teal">
                    ...
                    {activeTab === 'comments' && (
                      <div className="animate-fadeIn text-center py-16 bg-white/5 border border-dashed border-white/10 rounded-sm">
                         ...
                         <button className="mt-6 bg-yetomart-teal text-white px-8 py-3 rounded-sm font-black text-xs uppercase tracking-widest hover:bg-yetomart-teal/80 transition-all shadow-lg">Escrever Dúvida</button>
                      </div>
                    )}
                  </div>
                </div>
                  ...
                  return (
                    <button 
                      ...
                      className={`w-full p-5 rounded-sm flex items-start space-x-4 transition-all text-left group relative overflow-hidden ${
                        isActive ? 'bg-yetomart-teal/10 border border-yetomart-teal/30' : 'hover:bg-white/5 border border-transparent'
                      } ${isLocked ? 'opacity-40 grayscale' : ''}`}
                    >
                      {isActive && <div className="absolute left-0 top-0 bottom-0 w-1 bg-yetomart-teal shadow-[0_0_10px_rgba(34,87,122,0.8)]"></div>}
                      
                      <div className={`mt-1 flex-shrink-0 w-7 h-7 rounded-sm border flex items-center justify-center transition-all duration-300 ${
                        isActive ? 'bg-yetomart-teal border-yetomart-teal text-white shadow-[0_0_15px_rgba(34,87,122,0.4)]' : 'border-white/10 text-white/30 group-hover:border-white/30 group-hover:text-white'
                      }`}>
                        {isActive ? (
